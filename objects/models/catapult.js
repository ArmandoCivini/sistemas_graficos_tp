...
function createWheel(gl, glProgram, controlPoints, texture) {
    ...
    let wheel = new SweepClosedSurface(controlPoints, 4, new Circle(16, 0.1), gl, glProgram, [0,0,0], texture, 1/8, 1);
...
export function createCatapult(gl, glProgram, stoneTexture, woodTexture) {
    ...
    let floor = new SweepClosedSurface(controlPoints, 4, new Rectangle(0.5, 0.05), gl, glProgram, [0,0,0], woodTexture, 1, 2);
    floor.translate(0,-0.48,0);
    parent_node.addChild(floor);

    let frame = new SweepClosedSurface(controlPoints, 4, new Rectangle(0.1, 0.1), gl, glProgram, [0,0,0], woodTexture);
    ...
    let arm = new SweepClosedSurface(controlPoints, 4, new Rectangle(0.1, 0.1), gl, glProgram, [0,0,0], woodTexture);
    arm.scale(1, 1.2, 0.5);
    rotation_node.addChild(arm);

    let bucket = new SweepClosedSurface(controlPoints, 4, new Rectangle(0.3, 0.1), gl, glProgram, [0,0,0], woodTexture);
    ...
    let wheel1 = createWheel(gl, glProgram, controlPoints, stoneTexture);
    wheel1.translate(0.28,0.27,0);
    floor.addChild(wheel1);

    let wheel2 = createWheel(gl, glProgram, controlPoints, stoneTexture);
    wheel2.translate(-0.28,0.27,0);
    floor.addChild(wheel2);

    let wheel3 = createWheel(gl, glProgram, controlPoints, stoneTexture);
    wheel3.translate(0.28,-0.27,0);
    floor.addChild(wheel3);

    let wheel4 = createWheel(gl, glProgram, controlPoints, stoneTexture);
